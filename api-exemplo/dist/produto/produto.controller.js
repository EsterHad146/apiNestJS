...
let ProdutoController = class ProdutoController {
    ...
    findAll(res) {
        res.status(common_1.HttpStatus.OK).json(this.ProdutoService.findAll());
    }
    findOne(id, res) {
        res.status(common_1.HttpStatus.OK).json(this.ProdutoService.findOne(parseInt(id)));
    }
    create(body, res) {
        res.status(common_1.HttpStatus.CREATED).json(this.ProdutoService.create(body));
    }
    update(id, body, res) {
        res.status(common_1.HttpStatus.OK).json(this.ProdutoService.update(parseInt(id), body));
    }
    remove(id, res) {
        res.status(common_1.HttpStatus.NO_CONTENT).json(this.ProdutoService.remove(parseInt(id)));
    }
};
